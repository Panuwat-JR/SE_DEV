...
// 1. นำเข้า Link และเพิ่มไอคอน Eye, X, Upload, File มาใช้งาน
...
function Activities() {
  ...
  const [isEditModalOpen, setIsEditModalOpen] = useState(false); // คุมการเปิด/ปิดหน้าต่าง
  const [editingActivity, setEditingActivity] = useState(null); // เก็บข้อมูลแถวที่กดแก้ไข
  const [isCreateModalOpen, setIsCreateModalOpen] = useState(false);
  const [newActivity, setNewActivity] = useState({
    title: '', status: 'เปิดรับสมัคร', date_text: '', max_participants: 100, prize_money: '', fileName: ''
  });
  ...
  // ฟังก์ชันตอนกดปุ่ม "แก้ไข" (รูปดินสอ)
  const handleOpenEdit = (activity) => {
    setEditingActivity(activity); // จำข้อมูลกิจกรรมบรรทัดนั้นไว้
    setIsEditModalOpen(true);     // เปิดหน้าต่าง Modal
  };

  // ฟังก์ชันตอนกด "ยืนยันการแก้ไข" ในหน้าต่าง
  const handleSaveEdit = (e) => {
    e.preventDefault();
    alert(`จำลองการบันทึกข้อมูล: ${editingActivity.title} เรียบร้อย!`);
    ...
    <div className="relative h-full flex flex-col">
     ...
     {/* ================= 1. ส่วนหัวและปุ่มสร้าง ================= */}
      <div className="flex justify-between items-end mb-8 shrink-0">
        ...
        {/* ปุ่มสร้างกิจกรรมใหม่อยู่ตรงนี้ครับ */}
        <button 
            ...
            <tbody className="text-sm text-gray-700">
              ...
              {filteredActivities.map((activity) => (
                <tr key={activity.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors group">
                  ...
                  <td className="p-4">
                    {/* ปรับสี Badge ให้รองรับ "ดำเนินการสำเร็จ" */}
      ...
      {/* ================= หน้าต่าง Modal สำหรับแก้ไขกิจกรรม ================= */}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  >
                    ...
                    {/* เพิ่มตัวเลือก "ดำเนินการสำเร็จ" ใน Modal แก้ไข */}
                ...
                <div>
                  ...
                  <input 
                    type="text" required 
                    className="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-blue-500" 
                    value={editingActivity.date_text} 
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input 
                    ...
                    onChange={(e) => setEditingActivity({...editingActivity, max_participants: Number(e.target.value)})} 
                  />
                </div>
                <div>
                  ...
                  <input 
                    type="text" 
                    className="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-blue-500" 
                    value={editingActivity.prize_money} 
                    onChange={(e) => setEditingActivity({...editingActivity, prize_money: e.target.value})} 
                  />
                </div>
              </div>
      ...
      {/* ================= หน้าต่าง Modal สำหรับสร้างกิจกรรมใหม่ ================= */}
      {isCreateModalOpen && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl w-[500px] shadow-2xl overflow-hidden">
            <div className="flex justify-between items-center p-6 border-b border-gray-100">
            ...
            <form onSubmit={(e) => { 
              e.preventDefault(); 
              alert(`จำลองการสร้างกิจกรรม: ${newActivity.title} เรียบร้อย!`); 
              setIsCreateModalOpen(false); 
              // เคลียร์ค่าหลังจากบันทึก รวมถึง fileName
              setNewActivity({ title: '', status: 'เปิดรับสมัคร', date_text: '', max_participants: 100, prize_money: '', fileName: ''}); 
            }} className="p-6 space-y-4">
              
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">ชื่อกิจกรรม</label>
                <input 
                  type="text" required placeholder="เช่น โครงการ ELP..."
                  className="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-blue-500" 
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  >
                    ...
                    <option value="วางแผน">วางแผน</option>
                    <option value="กำลังดำเนินการ">กำลังดำเนินการ</option>
                    {/* เพิ่มตัวเลือก "ดำเนินการสำเร็จ" ใน Modal สร้างใหม่ */}
                    <option value="ดำเนินการสำเร็จ">ดำเนินการสำเร็จ</option>
                  </select>
                </div>
                <div>
                  ...
                  <input 
                    type="date" required 
                    className="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-blue-500" 
                    value={newActivity.date_text} 
                    onChange={(e) => setNewActivity({...newActivity, date_text: e.target.value})} 
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">เงินรางวัล</label>
                  <input 
                    type="text" placeholder="เช่น 10,000 บาท"
                    className="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-blue-500" 
                    value={newActivity.prize_money} 
                    onChange={(e) => setNewActivity({...newActivity, prize_money: e.target.value})} 
                  />
                </div>
              </div>

              {/* ส่วนที่เพิ่มใหม่: แนบเอกสาร */}
              <div className="pt-2">
