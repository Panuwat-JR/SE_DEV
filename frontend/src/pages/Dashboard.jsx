...
import { 
  ...
  Clock, Trophy, CheckCircle2, CircleDashed, ChevronRight, X, ChevronDown 
} from 'lucide-react';
import { Link } from 'react-router-dom';

function Dashboard() {      
  //step1: สร้างกล่องเปล่าวๆ ไว้เก็บข้อมูล
  ...
  const timelineData = {
    project_1: {
      ...
      steps: [
        ...
        { id: 4, title: "ดำเนินกิจกรรม", date: "15 ก.พ. 2569", status: "upcoming" },
        { id: 5, title: "สรุปผลโครงการ", date: "28 ก.พ. 2569", status: "upcoming" },
      ]
    },
    project_2: {
      name: "โครงการ ELP",
      steps: [
        ...
        { id: 4, title: "วันจัดกิจกรรมจริง", date: "20 มี.ค. 2569", status: "upcoming" },
      ]
    }
  };
    ...
    <div className="relative">
      ...
      <div className="grid grid-cols-4 gap-6 mb-8">
        <div className="bg-gradient-to-br from-blue-500 to-blue-600 text-white p-6 rounded-2xl shadow-sm relative overflow-hidden">
          ...
          <div className="text-3xl font-bold mb-1">{data.upcomingActivities?.length || 0}</div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-8">
          ...
          {/* ================= ไทม์ไลน์ (แก้ให้ขนาดพอดี ไม่ยืดเส้นยาว) ================= */}
          <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
            <div className="flex justify-between items-center mb-6">
              ...
              <div className="relative">
                <select 
                  className="appearance-none bg-white border border-gray-200 text-gray-700 text-sm rounded-lg pl-3 pr-8 py-1.5 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 font-medium cursor-pointer shadow-sm hover:bg-gray-50 transition-colors"
            ...
            <div className="relative pl-3 space-y-6 mt-2">
              {/* เส้นเชื่อมตรงกลาง (พื้นหลังสีเทา) - แก้ให้ความยาวพอดีกับเนื้อหา */}
              ...
              {currentTimeline.steps.map((step, index) => {
                ...
                // เช็คว่าเป็นอันสุดท้ายหรือเปล่า จะได้ไม่เว้นช่องว่างด้านล่างเยอะเกินไป
                const isLast = index === currentTimeline.steps.length - 1;

                return (
                  <div key={step.id} className="relative flex gap-4 items-start group">
                    {showGreenLine && (
                      <div className="absolute left-[7px] top-6 bottom-[-24px] w-[2px] bg-emerald-500 z-0"></div>
                    )}

                    <div className="relative z-10 bg-white pt-1">
                      {step.status === 'completed' ? (
                        <div className="w-4 h-4 rounded-full bg-emerald-500 text-white flex items-center justify-center ring-4 ring-white shadow-sm">
                          <CheckCircle2 size={16} className="fill-emerald-500 text-white" />
                        </div>
                      ) : step.status === 'current' ? (
                        <div className="w-4 h-4 rounded-full bg-blue-600 flex items-center justify-center ring-4 ring-white shadow-sm shadow-blue-600/30">
                          <Clock size={12} className="text-white" />
                        </div>
                      ) : (
                        <div className="w-4 h-4 rounded-full border-2 border-gray-200 bg-gray-50 flex items-center justify-center ring-4 ring-white">
